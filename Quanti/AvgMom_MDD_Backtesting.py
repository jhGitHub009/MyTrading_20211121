...
def GetSignal(dfPrice, avgNumMonth = 6 , shoulder=-5, knee=5):
    ...
    for idx in dfRefData.index:
        if thisStatus=='Hold':
            drawUp = GetDrawUp(dfRefData.loc[startIdx:idx]['AvgMoM'])  # 무릎만큼 올라가면.
            drawDown = GetDrawDown(dfRefData.loc[startIdx:idx]['AvgMoM'])  # 어깨만큼 내려가면.
        elif thisStatus == 'Buy':
            drawUp = 0.0
            drawDown = GetDrawDown(dfRefData.loc[startIdx:idx]['AvgMoM'])  # 어깨만큼 내려가면.
...
OECDLoc = 'USA'
saveFolder = './AvgMoM\Backtesting\AvgMomMDD\%s'%(keywordPrice)
shoulders = np.arange(-0.02, -2.0, -0.02)
knees = np.arange(0.02, 2.0, 0.02)
