...
def GetSignal(dfPrice, OECDURL, OECDLoc='USA', threshold = 0):
    ...
    dfRefData.loc[dfRefData['PCT']>= threshold,'Signal'] = 'Buy'
    dfRefData.loc[dfRefData['PCT']< threshold,'Signal'] = 'Sell'
    dfPrice['Signal'] = None
...
OECDLoc = 'USA'
saveFolder = './OECD\Backtesting\OECD_PCT\%s'%(keywordPrice)
oecdPCTs = np.arange(-1.0, 1.1, 0.01)
...
oecdPCTs = [round(oecdPCT,2) for oecdPCT in oecdPCTs]
summary = pd.DataFrame(columns=['OECD_PCT','Prec','Recall','Accuracy','FScore','AltCostScore','IniAsset', 'FinalAsset',
                                'CAGR','MDD'])
saveRawFolder = saveFolder + '\Raw'
if not isdir(saveRawFolder):
    makedirs(saveRawFolder)

for oecdPCT in oecdPCTs:
    print('%s --------------------'%(oecdPCT))
    dfPrice = GetSignal(dfPrice, OECDURL = OECDURL, OECDLoc = OECDLoc, threshold = oecdPCT)
    ...
    idxSum = len(summary)
    summary.loc[idxSum, 'OECD_PCT'] = oecdPCT
    ...
    summary.to_csv(saveFolder + '\SummaryOECDThreshold.csv', encoding='cp949',mode='w')
    dfAccount.to_csv(saveRawFolder + '\dfAccount_Threshold%s.csv'%(oecdPCT),encoding='cp949')
    dfAccuracy.to_csv(saveRawFolder + '\dfAccuracy_Threshold%s.csv'%(oecdPCT),encoding='cp949')
    del dfAccount
    del dfAccuracy
# del summary