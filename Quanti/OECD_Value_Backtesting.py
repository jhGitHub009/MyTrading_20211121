...
def GetSignal(dfPrice, valPercen):
    ...
    valThreshold = (dfRefData['Value'].max()-dfRefData['Value'].min())*valPercen/100
    valThreshold += dfRefData['Value'].min()
    ...
    dfRefData.loc[dfRefData['Value'] >= valThreshold, 'Signal'] = 'Buy'
    dfRefData.loc[dfRefData['Value'] < valThreshold, 'Signal'] = 'Sell'
    dfPrice['Signal'] = None
...
summary = pd.DataFrame(columns=['Value','Prec','Recall','Accuracy','FScore','AltCostScore','IniAsset',
                                'FinalAsset','CAGR','MDD'])
valPercen = np.arange(20,80,10)
valPercen = [round(value,0) for value in valPercen]
for value in valPercen:
    print('%s --------------------'%(value))
    dfPrice = GetSignal(dfPrice, value)
    ...
    idxSum = len(summary)
    summary.loc[idxSum, 'Value'] = value
    ...
    summary.to_csv('./OECD/Backtesting/OECDValue\SummaryOECDValue.csv', encoding='cp949',mode='w')
    dfAccount.to_csv('./OECD\Backtesting\OECDValue\Raw\dfAccount_Value%s.csv'%(value),encoding='cp949')
    dfAccuracy.to_csv('./OECD\Backtesting\OECDValue\Raw\dfAccuracy_Value%s.csv'%(value),encoding='cp949')
    del dfAccount
    del dfAccuracy
# del summary