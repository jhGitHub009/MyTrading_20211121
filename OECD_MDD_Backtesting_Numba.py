...
def GetDrawDownNumpy(nArr):
    _DrawDown = (nArr - np.maximum.accumulate(nArr)) / np.maximum.accumulate(nArr) * 100
    return _DrawDown[-1]
...
@jit(forceobj=True)
# @jit(forceobj=True, parallel=True)
def GetDrawUpNumpy(nArr):
    _DrawUp = (nArr - np.minimum.accumulate(nArr)) / np.minimum.accumulate(nArr) * 100
    return _DrawUp[-1]
...
# @cuda.jit
def _GetSignalNumpy(npRefData):
    # npRefData = dfRefData.to_numpy()
    # # dfRefData = np.genfromtxt(OECDURL, delimiter=',')
    # dfRefData = dfRefData[dfRefData[:,0]==OECDLoc]                          # dfRefData = dfRefData[dfRefData[0]==OECDLoc]
    # dfRefData = np.append(dfRefData, np.empty([dfRefData.shape[0], 1],dtype=object), axis=1)# dfRefData['DrawUp'] = None
    # dfRefData = np.append(dfRefData, np.empty([dfRefData.shape[0], 1], dtype=object), axis=1)# dfRefData['DrawDown'] = None
    # dfRefData = np.append(dfRefData, np.zeros([dfRefData.shape[0], 1]), axis=1)# dfRefData['Signal'] = 0

    startIdx = 0  # startIdx = dfRefData.index[0]
    thisStatus = 'Hold'
    for idx in range(0, npRefData.shape[0], 1):  # for idx in dfRefData.index:
        if thisStatus == 'Hold':
            drawUp = GetDrawUpNumpy(npRefData[startIdx:idx + 1, 6])  # 무릎만큼 올라가면.
            drawDown = GetDrawDownNumpy(npRefData[startIdx:idx + 1, 6])  # 어깨만큼 내려가면.
        elif thisStatus == 'Buy':
            drawUp = 0.0
            drawDown = GetDrawDownNumpy(npRefData[startIdx:idx + 1, 6])  # 어깨만큼 내려가면.
            ...
            drawUp = GetDrawUpNumpy(npRefData[startIdx:idx + 1, 6])  # 무릎만큼 올라가면.
            drawDown = 0.0
        npRefData[idx, -3] = drawUp
        npRefData[idx, -2] = drawDown

        if drawUp > knee:
            if thisStatus == 'Sell':
                startIdx = idx
            thisStatus = 'Buy'
            npRefData[idx, -1] = thisStatus
            ...
            npRefData[idx, -1] = thisStatus
    return npRefData
def GetSignalNumpy(dfPrice, OECDURL, OECDLoc, shoulder=-5, knee=5):
    ...
    dfRefData['Signal'] = 0
    npRefData = dfRefData.to_numpy()
    # npRefData = npRefData[:, 6].astype(np.float64)
    npRefData = _GetSignalNumpy(npRefData)

    dfRefData = pd.DataFrame(npRefData,columns = dfRefData.columns.tolist())
...
def GetSignal(dfPrice, OECDURL, OECDLoc, shoulder=-5, knee=5):
    ...
    dfRefData['TIME'] = pd.to_datetime(dfRefData['TIME'])
    # dfRefData['ReleaseDate'] = pd.to_datetime(dfRefData['TIME']) + datetime.timedelta(days=45)
    dfRefData['ReleaseDate'] = dfRefData['TIME'] + datetime.timedelta(days=45)
    if dfRefData.iloc[0]['ReleaseDate'] > dfPrice.index[0]:
        startDate = dfRefData.iloc[0]['TIME']
    else:
        startDate = dfPrice.index[0]
...
OECDLoc = 'USA'
saveFolder = './OECD\Backtesting\OECDKneeSholderTESTGPU\%s'%(keywordPrice)
shoulders = np.arange(-0.02, -2.0, -0.02)
knees = np.arange(0.02, 2.0, 0.02)
...
for shoulder in shoulders:
    for knee in knees:
        # if knee==0.01 and shoulder>=-1.66:
        #     continue
        print('------- knee : %s, shoulder : %s -------'%(knee, shoulder))
        start = timer()
        dfPrice = GetSignalNumpy(dfPrice, OECDURL = OECDURL, OECDLoc = OECDLoc, shoulder = shoulder, knee = knee)
        print("GetSignalNumba:", timer() - start)
        start = timer()
        dfPrice = GetSignal(dfPrice, OECDURL = OECDURL, OECDLoc = OECDLoc, shoulder = shoulder, knee = knee)
        print("GetSignal:", timer() - start)
